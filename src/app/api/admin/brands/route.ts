...
export async function GET() {
    try {
        ...
        return toResponse(e, "[admin.brands]");
    }
}
...
export async function POST(req: Request) {
    try {
        ...
        auditLog(authed.user.id, AUDIT_ACTIONS.BRAND_CREATED, "brand", brand.id, { name: body.name });
        return ok({ brand }, { status: 201 });
    } catch (e) {
        return toResponse(e, "[admin.brand.create]");
    }
}
